...
unconfoundedness_test <- function(
  ...
  if(!is.null(trim)){
    stopifnot(is.numeric(trim), length(trim)==2, trim[1] < trim[2],
              all(trim >= 0), all(trim <= 1))
  }

  # ---------------- Parse formula ----------------
  tt <- terms(formula)
  vars <- all.vars(formula)
  if(length(vars) < 2L) stop("Formula must be Y ~ A + Xs.")
  y_name <- vars[1L]
  rhs_terms <- attr(tt, "term.labels")
  if(length(rhs_terms) < 1L) stop("Formula RHS must include treatment A (binary).")
  a_name <- strsplit(rhs_terms[1L], ":|\\*")[[1L]][1L]  # first symbol treated as A

  req <- unique(c(vars, a_name, y_name))
  if(!all(req %in% names(data_rct))) stop("Missing formula columns in data_rct.")
  if(!all(req %in% names(data_obs))) stop("Missing formula columns in data_obs.")
  ...
  coerce_A <- function(df, a){
    ...
    if(is.character(v)){
      uv <- unique(v); if(length(uv)!=2) stop("Character A must have 2 levels.")
      v <- as.integer(v==uv[2L])
    }
    if(is.factor(v)) v <- as.integer(v==levels(v)[2L])
    if(!is.numeric(v)) stop("A must be numeric/logic/factor/character 2-level.")
    if(!all(v %in% c(0,1))){
      u <- sort(unique(v)); stop(sprintf("A must be 0/1. Found: %s", paste(u, collapse=", ")))
    }
    df[[a]] <- v; df
  }
  ...
  Y_r <- mf_rct[[y_name]]; A_r <- mf_rct[[a_name]]
  Y_o <- mf_obs[[y_name]]; A_o <- mf_obs[[a_name]]
  if(family_y == "binomial"){
    if(!all(Y_r %in% c(0,1)) || !all(Y_o %in% c(0,1))){
      stop("With family_y='binomial', Y must be 0/1 in both datasets.")
    }
  } else {
    if(!is.numeric(Y_r) || !is.numeric(Y_o)) stop("With family_y='gaussian', Y must be numeric.")
  }
  ...
  auto_detect_shift_ks <- function(Xr, Xo, alpha){
    ...
    if(length(cols) == 0L) {
      return(list(available=TRUE, pvals=NULL, padj=NULL, reject_any=FALSE, rejected_cols=character(0)))
    }
    pvals <- vapply(cols, function(cn){
      suppressWarnings(stats::ks.test(Xr[,cn], Xo[,cn])$p.value)
    }, numeric(1))
    padj <- stats::p.adjust(pvals, method = "BH")
    reject <- padj < alpha
    list(available=TRUE, pvals=pvals, padj=padj,
  ...
  auto_detect_shift_energy <- function(Xr, Xo, alpha, R){
    ...
    list(available=TRUE, p_value=et$p.value, reject = (et$p.value < alpha), R=R)
  }

  if(transport == "auto"){
    ks_res <- if(auto_method %in% c("both","ks")) auto_detect_shift_ks(X_r, X_o, auto_alpha) else NULL
    en_res <- if(auto_method %in% c("both","energy")) auto_detect_shift_energy(X_r, X_o, auto_alpha, auto_energy_R) else NULL

    detected <- FALSE
    used <- character(0)
    if(!is.null(ks_res) && ks_res$available){
      if(ks_res$reject_any){ detected <- TRUE; used <- c(used, "ks") }
    }
    if(!is.null(en_res) && en_res$available){
      if(en_res$reject){ detected <- TRUE; used <- c(used, "energy") }
    }
  ...
  density_ratio_weights <- function(Xr, Xo){
    ...
    if("(Intercept)" %in% names(df)) df[["(Intercept)"]] <- NULL
  ...
  w_rct <- if(transport_applied == "rct_to_obs") density_ratio_weights(X_r, X_o) else rep(1, nrow(X_r))
  ...
  fit_ps <- function(A, X, samp_w){
    ...
    if("(Intercept)" %in% names(df)) df[["(Intercept)"]] <- NULL
    suppressWarnings({
      fit <- stats::glm(A ~ ., data = df, family = stats::binomial(), weights = samp_w)
    })
  ...
  fit_outcome_models <- function(Y, A, X, family, samp_w){
    ...
    df1 <- data.frame(Y = Y[A==1], X[A==1, , drop=FALSE]); w1 <- samp_w[A==1]
    df0 <- data.frame(Y = Y[A==0], X[A==0, , drop=FALSE]); w0 <- samp_w[A==0]
    if("(Intercept)" %in% names(df1)) { df1[["(Intercept)"]] <- NULL; df0[["(Intercept)"]] <- NULL }
    f1 <- suppressWarnings(stats::glm(Y ~ ., data=df1, family=fam, weights=w1))
    f0 <- suppressWarnings(stats::glm(Y ~ ., data=df0, family=fam, weights=w0))
    type <- "response"
  ...
  ate_ipw <- function(Y, A, e, samp_w, stabilize=TRUE, trim=NULL){
    ...
    w_all <- w_t + w_c
    if(!is.null(trim)){
      ql <- stats::quantile(w_all, trim[1]); qh <- stats::quantile(w_all, trim[2])
      keep <- (w_all >= ql) & (w_all <= qh)
      Y <- Y[keep]; w_t <- w_t[keep]; w_c <- w_c[keep]
    }
  ...
  ate_aipw <- function(Y, A, e, m1, m0, samp_w, stabilize=TRUE, trim=NULL){
    ...
    if(!is.null(trim)){
      ql <- stats::quantile(base_w, trim[1]); qh <- stats::quantile(base_w, trim[2])
      keep <- (base_w >= ql) & (base_w <= qh)
      Y <- Y[keep]; A <- A[keep]; e <- e[keep]; m1 <- m1[keep]; m0 <- m0[keep]; samp_w <- samp_w[keep]
    }
    numer <- if(stabilize) stab_num(A) else 1
    pseudo <- m1 - m0 + numer * ( A*(Y - m1)/e - (1-A)*(Y - m0)/(1-e) )
    W  <- samp_w; Sw <- sum(W)
    tau <- sum(W * pseudo) / Sw
  ...
  one_effect <- function(df, family, est, samp_w){
    ...
    Y <- mf[[y_name]]; A <- mf[[a_name]]
    X <- model.matrix(form_X, df)
    ps <- fit_ps(A, X, samp_w)
    e  <- ps$p
    if(est == "ipw"){
      ipw <- ate_ipw(Y, A, e, samp_w = samp_w, stabilize=stabilize, trim=trim)
      list(ate = ipw$tau, var = NA_real_, IF = NULL,
           details = list(ps_model=ps$fit, e=e, method="ipw"))
    } else {
      outs <- fit_outcome_models(Y, A, X, family, samp_w)
      aipw <- ate_aipw(Y, A, e, outs$m1, outs$m0, samp_w = samp_w,
                       stabilize=stabilize, trim=trim)
      v <- sum(aipw$IF^2)  # var(Σ IF_i) since IF already scaled by W/Sw
      list(ate = aipw$tau, var = v, IF = aipw$IF,
           details = list(ps_model=ps$fit, out1=outs$fit1, out0=outs$fit0,
                          e=e, m1=outs$m1, m0=outs$m0, method="aipw"))
    }
  }
  ...
  if(B < 200L) warning("Low bootstrap reps; consider B >= 500 for stable CIs.")
  boot_diffs <- numeric(B)
  n_r <- nrow(data_rct); n_o <- nrow(data_obs)
  for(b in seq_len(B)){
    ...
    wr_b <- if(transport_applied == "rct_to_obs") density_ratio_weights(Xr_b, Xo_b) else rep(1, length(idx_r))
    eb_r <- one_effect(data_rct[idx_r, , drop=FALSE], family_y, estimator, samp_w = wr_b)$ate
    eb_o <- one_effect(data_obs[idx_o, , drop=FALSE], family_y, estimator, samp_w = rep(1, length(idx_o)))$ate
    boot_diffs[b] <- eb_o - eb_r
  }
  ci <- stats::quantile(boot_diffs, probs = c(alpha/2, 1 - alpha/2), names = FALSE)
  ...
  diag_overlap <- function(A, e){
    rng <- range(e); extreme <- mean(e < 0.01 | e > 0.99)
    list(ps_min=rng[1], ps_max=rng[2], prop_extreme=extreme)
  }
  diag_r <- diag_overlap(model.frame(formula, data_rct)[[a_name]], est_r$details$e)
  diag_o <- diag_overlap(model.frame(formula, data_obs)[[a_name]], est_o$details$e)
  ...
  out$summary <- data.frame(
    ...
    Wald_z = z, Wald_p = p_wald,
    n_RCT = nrow(data_rct), n_OBS = nrow(data_obs),
    row.names = NULL
  )
...
print.unconf_test <- function(x, ...){
  ...
  cat(sprintf("- Estimator: %s; Outcome family: %s; Transport requested: %s; Applied: %s\n",
              x$estimator, x$family_y, x$transport, x$transport_applied))
  cat(sprintf("- ω̂_RCT = %.6f; ω̂_OBS = %.6f; Δ = %.6f\n",
              x$rct_effect, x$obs_effect, x$diff_obs_minus_rct))
  cat(sprintf("- Bootstrap CI for Δ: [%.6f, %.6f]\n", x$bootstrap_CI[1], x$bootstrap_CI[2]))
  cat(sprintf("- Wald test: z = %.3f, p = %.4g (H0: Δ = 0)\n", x$wald$z, x$wald$p_value))
  cat("- Positivity diagnostics (PS in [min,max], extreme PS <1% or >99%):\n")
  cat(sprintf("  RCT: [%.3f, %.3f], extreme=%.2f%%;  OBS: [%.3f, %.3f], extreme=%.2f%%\n",
              x$diagnostics$rct_ps$ps_min, x$diagnostics$rct_ps$ps_max, 100*x$diagnostics$rct_ps$prop_extreme,
              x$diagnostics$obs_ps$ps_min, x$diagnostics$obs_ps$ps_max, 100*x$diagnostics$obs_ps$prop_extreme))
  if(!is.null(x$diagnostics$transport)){
    tr <- x$diagnostics$transport
    cat("- Transport (RCT->OBS) weights summary:\n")
    cat(sprintf("  mean=%.3f, min=%.3f, q01=%.3f, median=%.3f, q99=%.3f, max=%.3f, ESS=%.1f\n",
                tr$w_mean, tr$w_min, tr$w_q[1], tr$w_q[2], tr$w_q[3], tr$w_max, tr$ESS))
  }
  if(!is.null(x$diagnostics$auto)){
    ad <- x$diagnostics$auto
    cat(sprintf("- Auto shift detection: detected=%s, method=%s, alpha=%.3f\n",
                ad$detected, ad$requested_method, ad$alpha))
    if(!is.null(ad$ks)){
      ks <- ad$ks
      if(ks$available && length(ks$padj)){
        bad <- if(length(ks$rejected_cols)) paste(ks$rejected_cols, collapse=", ") else "none"
        cat(sprintf("  KS: any_reject=%s; rejected_cols=%s\n", ks$reject_any, bad))
      } else {
